...
param (
	...
	[string]
	$LogPath = "C:\Temp\demo\complex.csv"
)

$ErrorActionPreference = 'Stop'
trap {
	Write-Log -Message "Script failed" -ErrorRecord $_ -Status ERROR
	Stop-Log
	throw $_
}
...
Write-Log -Message "Retrieving users from Entra"
$users = Invoke-EntraRequest -Path users
Write-Log -Message "$($users.Count) users found"
foreach ($user in $users) {
	Write-Log -Message "Updating department for user $($user.UserPrincipalName) to $($Department)" -Target $user.UserPrincipalName
