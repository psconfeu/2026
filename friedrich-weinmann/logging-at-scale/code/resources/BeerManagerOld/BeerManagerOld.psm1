...
$logFolder = Join-path $env:LocalAppData "PowerShell\BeerManager\Logs"
if (-not (Test-Path -Path $logFolder)) {
	$null = New-Item -Path $logFolder -ItemType Directory -Force
}
Start-Log -Path "$logFolder\BeerManager-$(Get-Date -Format 'yyyy-MM-dd')-$($PID).log"
...
function Resolve-Beer {
	...
	Write-Log -Message "Resolving preferred beer for $User" -Target $User -Tags Beer
	$beer = "Hofbräu","Becks","Augustiner" | Get-Random
	Write-Log -Message "Preferred Beer of $User is $beer" -Target $User -Tags Beer
